...
namespace Utility
{
    ...
    public static class BusinessRules
    {
        ...
        public static string RegisterError(int errorCode)
        {
            ...
            if(errorCode == -100)
            {
                returnValue = "Student cannot register for a course in which there is already an ungraded registration.";
            }
            else if (errorCode == -200)
            {
                returnValue = "Student has exceeded maximum attempts on mastery course";
            }
            else if (errorCode == -300)
            {
                returnValue = "An error has occurred while updating the registration";
            }
            else if(errorCode != -300 ||  errorCode != -200 ||  errorCode != -100)
            {
        ...
        /// REturns
        /// </summary>
        /// <param name="stringToBeParsed"></param>
        /// <param name="parseAtIndex"></param>
        /// <returns></returns>
